...
// Model bazlı satış verileri API'si
app.get('/api/model-bazli-satis', (req, res) => {
    const query = 'SELECT model_adi, satis_sayisi FROM model_bazli_satis';
    db.query(query, (err, results) => {
        if (err) {
            console.error('Veri çekme hatası:', err);
            res.status(500).send('Veritabanı hatası');
...
// Şarj istasyonu kullanım verileri API'si
app.get('/api/sarj-kullanim', (req, res) => {
    const query = 'SELECT ay_id, kullanim_miktari FROM sarj_istasyon_kullanimi ORDER BY FIELD(ay_id, "Ocak", "Şubat", "Mart", "Nisan", "Mayıs", "Haziran", "Temmuz", "Ağustos", "Eylül", "Ekim", "Kasım", "Aralık")';
    db.query(query, (err, results) => {
        if (err) {
            console.error('Veri çekme hatası:', err);
            res.status(500).send('Veritabanı hatası');
